loadExploreR <- function(data, dat, input, output) {
  dat2$yChoices <<- c("Population (%)",
          "Population (Count)",
          "Spend (Total, GBP)",
          "Spend (Per Capita, GBP)",
          "Activity (Total)",
          "Activity (Per Capita)")
  ...
  output$oneCoverBoxDate13 <- renderUI({h4(paste0("Activity from ", dat2$mintime, " to ", dat2$maxtime))})
  ...
  output$pyramidActivity <- renderPlotly({
    ggplotly(ggplot(as.data.frame(
      ...
      data[,selectedGroup] <- data[,selectedGroup] %>% makeNice()
      data$text <-
