...
server <- function(input,output,session){
  ...
  observeEvent(input$zeroThreeProceed,{
    ...
    #######################################################################################################################################################
    # What happens if we assume no missing values?
    
    # activity$cost <- lapply(activity$cost%>% unlist(), function(x) {if (is.na(x)) {0} else {x}}) %>% unlist()
    # activity$spec <- lapply(activity$spec%>% unlist(), function(x) {if (is.na(x)) {"Unknown"} else {x}}) %>% unlist()
    # 
    # for(i in colnames(activity)[2:(ncol(activity) - 4)]) {
    #   activity[[i]][is.na(activity[[i]])] <- "No Further Levels"
    # }
...
# browser()
# TODO from here, check where Error in $: $ operator is invalid for atomic vectors occurs
    ...
    df <- df[!is.na(df$id),]
    print("Saved files")
    # This needs to be added to dat2 when possible #TODO, important
    #  dat2$actLookUp <- rbind(df,data.frame(id = which(!dat$attributes$id%in%ids), lookUp = -1))
    actLookUp <- rbind(df,data.frame(id = which(!attributes$id%in%ids), lookUp = -1))
    # browser()
    ####
...
# TODO supress small numbers in analysis/cohort?
# TODO: fix help on theoplots UI + colour selection needs update
# TODO upload powerpoint to github + ready presentation with survery link - in chat on teams