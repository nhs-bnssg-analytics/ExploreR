...
source("./body/theoplot/theoplot.R", local = TRUE)
...
body <- dashboardBody(tags$script(HTML("$('body').addClass('fixed');")),
    ...
    ')),
                        ...
                        tabItem(tabName="oneClinic",
                                fluidRow(
                                  box(h4("Clinical Characteristics"),
                                  ...
                                  column(width = 10,
                                         fluidRow(
                                           box(width = 12, #title = "Clinical Characteristics",
                                               # h3("Multimorbidity in the population"),
                                               fluidRow(
                                                 column(width = 6,
                                                        selectInput("oneClinY", "Y Axis Field:", choices = c(1,2)),
                                                        selectizeInput("oneClinGroupBy", "Split data further by (optional):", choices = c(1,2),
                                                                       options = list(
                                                 ...
                                                 column(width = 6,
                                                        # uiOutput("collapseFilterOutput2")
                                                 ),
                                               ),
                                         ...
                                         fluidRow(
                                           div(
                                             box(width = 12,
                                                 ...
                                                 p(" "),
                                                 # p(" ", actionLink(inputId = "viz_helpClinData",
                                         ...
                                         fluidRow(
                                           div(
                                             box(width = 12,
                                                 ...
                                                 # p("Please note spending per head is per patient with relevant activity - i.e. individuals with no activity in the system are not accounted for in the per head scaling"),
                                  ...
                                  column(width = 10,
                                         fluidRow(
                                           box(width = 12, #title = "Deprivation",
                                               ...
                                               selectizeInput("oneDeprivationGroupBy", "Split data further by (optional):", choices = c(1,2),
                                                              options = list(
                                                                placeholder = 'Please select an option below',
                                                                onInitialize = I('function() { this.setValue(""); }')
                                                              )
                                         ...
                                         fluidRow(
                                           div(
                                             box(width = 12, 
                                                 ...
                                                 # p("Socio Wd Imd Decile 19 is the Index of Multiple Deprivation (from 2019)."),
                                                 # p("To see deprivation on a map of the BNSSG, go to Wider Determinants tab."),
                                                 # p(" ", actionLink(inputId = "DepDataCalc",
                                                 #                   label = HTML("How is the data calculated?"),
                                                 #                   icon = icon("info-circle"),
                                                 #                   style='font-size:100%')),
                                  ...
                                  column(width = 10,
                                         fluidRow(
                                           box(width = 12, #title = "Geography",
                                               # p("Set up hierarchy for area. fields / some graphs about attributes and activity by area"),
                                               # selectInput("oneGeoX", "Current Area Selected (X-axis/area field in previous pages):", choices = c(1,2)),
                                               selectInput("oneGeoX", "Selected Area/Geographical Field:", choices = c(1,2)),
                                               selectInput("oneGeoY", "Y Axis Field:", choices = c(1,2)),
                                               selectizeInput("oneGeoGroupBy", "Split data further by (optional):", choices = c(1,2),
                                                              options = list(
                                                                placeholder = 'Please select an option below',
                                                                onInitialize = I('function() { this.setValue(""); }')
                                                              )
                                         ...
                                         fluidRow(
                                           div(
                                             box(width = 12,
                                                 ...
                                                 p("Use the dropdowns to change the information displayed."),
                                                 # p("If deployed, the Map Tool provides a superior graphical view of these numbers."),
                                                 # p(" ", actionLink(inputId = "AreaDataCalc",
                                                 #                   label = HTML("How is the data calculated?"),
                                                 #                   icon = icon("info-circle"),
                                                 #                   style='font-size:100%')),
                                  ...
                                  column(width = 10,
                                         fluidRow(
                                           box(width = 12, # title = "Wider Determinants",
                                               ...
                                               selectizeInput("oneWidDetGroupBy", "Split data further by (optional):", choices = c(1,2),
                                                              options = list(
                                                                placeholder = 'Please select an option below',
                                                                onInitialize = I('function() { this.setValue(""); }')
                                                              )
                                               ...
                                               # br(),
                                               # br(),
                                               # p(h3("Map of LSOAs by determinant")),
                                               # plotlyOutput("oneWidDetPlot1"),
                                               # selectInput("oneWidDetMapRegion", "At geographical resolution:", choices = list("Stacked" = TRUE, "Separate" = FALSE)),
                                               # leafletOutput("oneWidDetMapPlot", height = "800px")
                                           ),
                                         )
                                  ),
                                  column(width = 2,
                                         fluidRow(
                                           div(
                                             box(width = 12,
                                                 ...
                                                 p("Use the dropdowns to change the information displayed."),
                                                 # p("The second graph (map of BNSSG) is only effected by the selected determinant."),
                                                 ...
                                                 # p("If deployed, the Map Tool provides a superior graphical view of these numbers."),
                                                 # p(" ", actionLink(inputId = "WiderDataCalc",
                                                 #                   label = HTML("How is the data calculated?"),
                                                 #                   icon = icon("info-circle"),
                                                 #                   style='font-size:100%')),
                                                 ...
                                                 p(" ", actionLink(inputId = "modalGraphInfoWider",
                                                                   ...
                                                                   style='font-size:100%')),
                                                 # p("Notes on the map:"),
                                                 # p("The map is interactive. Please take careful note of the determinant being displayed; the colour shading might not",
                                                 #   " be intuitive.")
                                                 # )
                                             ), style = "position:fixed;  width:inherit;")
                                         )
                        ...
                        tabItem(tabName="twoLTCAge",
                                fluidRow(
                                  box(h4("Chronic Conditions by Age"),
                                  ),
                                  box(background = "green", width = 4,
                        ...
                        tabItem(tabName="fiveTab",
                                fluidRow(
                                  box(h4("Risk Stratification and Forecasting via Regression"),
                                  ),
                                  box(background = "green",width = 4,
                                         ...
                                         fluidRow(
                                           box(width = 12, collapsible = TRUE, title = "Variable Setup",
                                               ...
                                               column(width = 12,
                                                      ...
                                                      #          # p(" ", actionLink(inputId = "twoRisk1Text6",
                                                      #          #                   icon = icon("exclamation-triangle"),#triangle-exclamation
                                                      #          #                   label = HTML(""))),
                                                      #          # bsTooltip("twoRisk1Text6", title = "It is recommended these settings are left as default.")
                                                      #   ),
                                                      # ),
