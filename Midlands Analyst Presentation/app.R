  ...
  observeEvent(input$zeroAnalysisDatasetGetIDs,{
    ...
    if(length(which(sapply(dat2$AnalysisDataset$rules, function(x) {
      ...
      if(length(which(analysisSet)) > 20) {
        ...
        print("Got data")
        # browser()
        # TODO dat2 gets massively bloated
        # and also throw a window while saving down the new set
...
# TODO suppress small numbers in analysis/cohort?
# TODO: fix help on theoplots UI
# TODO go through LTC and BtH definitions using SWD every column on Monday with Anna
# Define BtH using only QOF conditions? 

# Adding an analysis dataset crashes the program
# Cohort UI loads too soon - need to force a reload
# LTC by age crashes the program

# TODO for external users remove upload IDs functions (all pages)


# TODO w/ Rich put into a document the points of discussion on IG/DPIA and that we have considered risk
# ALso, SWD already has a DPIA - email from PAul Roy - not necessary to have a new one
# DPIA is only necessary in situations where a new dataflow is involved as we have an overarching DPIA in place for use of the extracted primary care data 

# TODO HR for leave

# TODO look at pathsimR (on github) to figure out how to create downloadable pdfs

# TODO social care data - which event code is which
# Analysis on can we find patterns in expensive events

# TODO powerBI + how to use/license/access
