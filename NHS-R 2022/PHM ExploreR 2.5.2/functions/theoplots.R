...
constructHoverText2 <- function(row){
  ...
  text = paste0(
    ...
    "Duration: ", difftime(row["dep_date"], row["arr_date"], units = "days")," days ", "<br>",
    q,# "<br>",
    "Specialty: ",  makeNice(row["spec"]), "<br>",
    "Cost (GBP): ", format(round(as.numeric(row["cost"])),big.mark = ",")
  )
...
theoData <- function(id, dat, dat2, tree = input$fourTheotree, att = T) {
  ...
  output$fourTheoSingleTheoActivity <- renderUI({
    list(p(paste0(nrow(d), " instances of activity. Total cost: ", round(sum(d$cost),2), "GBP")))
  })
  ...
  if(!is.null(input$fourTheoMultiActivityToPlot)) {
    tryCatch({
      ...
      if(nrow(data) != 0) {
        ...
        print(Sys.time())
        # browser()
        # print(paste0(length(unique(data3$id)) * 30 + 80, "px"))
...
plotMultiLineTheoDef <- function(dat, dat2, input, id) {
  ...
  if(!is.null(input$fourTheoMultiActivityToPlot)) {
    tryCatch({
      ...
      if(nrow(data) != 0) {
        ...
        data2$colour <- sapply(data2$filterNames, function(x) {
          c("green", "blue", "yellow", "black", "red", "grey")[as.numeric(
            input[[paste0(dat2$multiTheoColS[startsWith(x, dat2$multiTheoColS)],  paste0(rep("a",dat2$multiTheoColSNum), collapse = ""))]]
          )]
        }) %>% unlist()
        data3 <- data2
        data3$id<-as.character(data3$id)
        ...
        pal <- data3$colour
        pal <- setNames(pal, data3$filterNames)
