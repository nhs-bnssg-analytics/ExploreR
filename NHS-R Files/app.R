...
server <- function(input,output,session){
      ...
      if (input$zeroRad == "Local Files") {
        fluidRow(
          ...
          column(width = 12,
                 br(),
                 br(),
                 actionButton("loadFileData", "Load Data"),
                 br()
          ))
  ...
  observeEvent(input$zeroThreeProceed,{
    # browser()
    # preprocess
    nums <- attributes %>% select(starts_with("clinic.")) %>% select(!starts_with("clinic.misc")) %>% colnames()
    for (i in nums) {
      attributes[,i] <- lapply(attributes[,i], function(x){
        if (is.na(x)) {0} else {x}
      }) %>% unlist()
    }
    activity$cost <- lapply(activity$cost%>% unlist(), function(x) {if (is.na(x)) {0} else {x}}) %>% unlist()
    
    source("./functions/act_format.R")
    source("./functions/att_format.R")
    # browser()
    attributes <- att(attributes, activity, input,output)
    dat <<- left_join(attributes, process_act(activity)) %>% list(attributes = ., activity = activity)
  ...
  observeEvent(input$zeroAnalysisDatasetGetIDs,{
    ...
    if(length(which(sapply(dat2$AnalysisDataset$rules, function(x) {
      ...
      dat2$AnalysisDatasets[[input$zeroAnalysisDatasetName]] <<- getAnalysisDataset(input,dat,dat2$AnalysisDataset)
      print("Got data")
      ...
      output$zeroAnalysisDataText <- renderText(
        paste0(
          ...
          # sum(dat$attributes$total_act[dat2$AnalysisDatasets[[input$zeroAnalysisDataPicker]]]),
  ...
  observeEvent(input$zeroAnalysisDataUploadIdsButton, {
    ...
    if (ext == "csv") {
      ...
      if(ncol(dat2$cohortIDs) == 1 && nrow(dat2$cohortIDs) > 0) {
        ...
        output$zeroAnalysisDataGetIDsMessage <- renderText(paste0("Successfully identified ", 
                                                          length(which(dat2$AnalysisDatasets[[input$zeroAnalysisDataName]])),
                                                          " individuals in dataset."))
      }
    }
  })
...
##############################################
  ...
  observeEvent(input$navTabGlobalVarAdd, {
    if(input$navTabGlobalVar %in% dat2$groupByList) {
      ...
      if(length(unique(dat$attributes[[input$navTabGlobalVar]])) < 11) {
  ...
  observeEvent(input$navTabGlobalVarRemove, {
    if(input$navTabGlobalVar %in% dat2$groupByList) {
      i <- which(dat2$groupByList == input$navTabGlobalVar)
      dat2$groupByListText <<- dat2$groupByListText[-i]
      dat2$groupByList <<- dat2$groupByList[-i]
      updateAllWithNewImportantField(dat)
      output$navTabMessage <- renderText(paste0("Removed ", makeNice(isolate(input$navTabGlobalVar))))
    } else {
      output$navTabMessage <- renderText(paste0("Could not remove ", makeNice(isolate(input$navTabGlobalVar)), " as variable not found in GlobalGroups."))
    }
  })
...
#     })
  ...
  observeEvent(input$clin3x3Plot, {
    ...
    if(!is.null(input$twoLTCAgeLTCSelection)) {
      ...
      # paste0("util.segmentation.",input$twoClusterName)
    ...
    if(toLoad$segBTH) {
      if("util.segmentation.BtHSegment"%in%colnames(dat$attributes)) {
        ...
        table <- cbind(bridges_to_health(dat, groupLookUp(dat2, "")), dat$attributes %>%
                         group_by(util.segmentation.BtHSegment,util.segmentation.BtHDefinition) %>% 
        ...
        output$twoBthTreeMap <- renderPlotly({
          ...
          ) # %>% layout(title = "Number Of Individuals In Segments")
        })
  ...
  # TODO Check this front if they are correctly added to dat2
  # ## Add analysis dataset
  # observeEvent(input$zeroAnalysisDataPickerAdd, {
  #   # Render sidebar to show file upload options
  #   output$tabs <- renderMenu({
  ...
  #   dat2$AnalysisDataset$join <<- list()
  # })
  # TODO server problem: I think it's to do with this loading before the loadExploreR function.
  ...
  output$threeCohortDownloadButton <- downloadHandler(
    filename = function() {
      paste(input$threeIDName, ".csv", sep = "")
    },
    content = function(file) {
      write.csv(dat$attributes$id[dat$attributes[[paste0("util.cohort.", input$threeIDName)]] == "Yes"], file, col.names = F,row.names = F)
    }
  )
 ...
 observeEvent(input$theoFileInput, {
   ...
   if (ext == "csv") {
     dat2$theoFile <<- read.csv(input$theoFileInput$datapath,stringsAsFactors = F)
     ...
     if(n > 4 | n < 1) {
       ...
       dat2$theoFile <<- dat2$theoFile[dat2$theoFile[,1]%in%dat2$actLookUp$id,]
 ...
 observeEvent(input$fourTheoSingleInfo1, {
   showModal(
     modalDialog(
       ...
       p("To get started, select the data range of interest (or leave as default). Then, use one of the tabs below to select an individual, and click plot."),
 ...
 observeEvent(input$theoFileInputMulti, {
   ...
   if (ext == "csv") {
     ...
     if(n > 4 | n < 1) {
       ...
       # Handle file
       # Assume columns contain correct data types
       dat2$theoFileMulti <<- dat2$theoFileMulti[dat2$theoFileMulti[,1]%in%dat2$actLookUp$id,]
       if(n > 1) {
         for(i in 2:n) {
       ...
       if(nrow(dat2$theoFileMulti) > 0) {
         output$theoFileUIMulti <- renderUI({
           fluidRow(
             column(width = 12,
                    br(),
                    selectInput("fourTheoFileSearchIDMulti", "Select ID:", choices = dat2$theoFileMulti[,1]),
                    # textInput("fourTheoFileIDMulti", "Type to Select:", value = dat2$theoFileMulti[,1][1]),
                    column(width = 4,
 ...
 observeEvent(input$fourTheoMultiInfo1, {
   showModal(
     modalDialog(
       ...
       p("To get started, select the data range of interest (or leave as default)."),
 ...
 observeEvent(input$fourTheoMultiActivityToPlot, {
    ...
    if(!is.null(input$twoRisk1Var1) & nrow(ntree[treeCols,]) > 0) {
      ...
      # browser()
        ...
        if(input$twoRisk1Var2 == "Likelihood of activity [0,1]") {
          ...
          # Nick says bad programming to do this - revisit later TODO
  ...
  observeEvent(input$fiveRisk1InfoD3, {
    showModal(
      modalDialog(
        ...
        # p("6. A summary of how many NAs were found, and subsequently excluded."),
  ...
  updateAllWithNewImportantField <- function(dat) {
    ...
    updatePickerInput(session, "fourTheSingleGlobalGroupValues", choices = t, selected = input$fourTheSingleGlobalGroup)
  }
...
# TODO ExploreR access for GPs resilience

# TODO SERVER Notes: hold left click to close select options
# If crashes on new analysis dataset, reload program, changes made should carry over despite crash


# TODO: re-structure data field setting page - currently very outdated - TICK?
# TODO: fix help on theoplots UI + colour selection needs update
# TODO: make loading part faster (of dat2 - turns out, lapplys take up a lot of memory - remove them / use more mutates with custom functions) - partial TICK
# TODO: guide - add in more about how dates need to be in yyyy/mm/dd format - TICK
# Updated act data to aggregate over last year, but use more for theoplots
# TODO delete previous activity snippets when loading data