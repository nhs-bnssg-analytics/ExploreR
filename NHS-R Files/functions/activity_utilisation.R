#utilisation by demographic and pod

# util: comparison options include highlighted areas; comparator group is BNSSG

# ntree <- dat2$theorows[-22,]%>%as.data.frame()
# t <- sapply(1:nrow(ntree), function(i) {
#   paste0(ntree[i,!is.na(ntree[i,])], collapse = ".")
# })
# 
# treeCols <- which(t %in% (get_selected(input$twoSegGraphsActTree, format = "names") %>%
#                             lapply(function(x){
...
# plot(act%>%group_by(arr_date)%>% summarise(cost = sum(cost)))
...
# as.data.frame(distinct(dat2$theorows[-22,-3]))

# d <- lapply(d, function(x){
#   keep <- paste0(x[, dat2$actPodCols], collapse = ".") %in% selected
#   x[keep,]
# })
