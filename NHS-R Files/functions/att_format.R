...
gc()
  ...
  attributes <- attributes[,!names(attributes)%in%c("demog.sex", "demog.age")]
  # browser()
...
dep <- function(input, dat, area = input$zeroFieldAssignSelectlocalit, saveAct = T) {
  gc()
  ...
  dat2$groupByList <- list("util.ltc_sum", dat2$dep, "util.age_band_10", dat2$area[1], dat2$ethnicity, dat2$wider[1], if("util.segmentation.BtHSegment"%in%colnames(dat$attributes)) {"util.segmentation.BtHSegment"} else {NA})
  dat2$groupByListText <- list("LTC Count", "Deprivation", "Age Bands (10)", if(area!=input$zeroFieldAssignSelectlocalit) {"PCN"} else {"ICP (current localities)"},"Ethnicity","Wider Determinants","BtH Segments")
