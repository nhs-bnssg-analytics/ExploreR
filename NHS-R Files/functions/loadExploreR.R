loadExploreR <- function(data, dat, input, output) {
  ...
  # output$oneCoverBox2 <- renderUI({h4(tags$b("Selected Dataset: "), makeNice(input$zeroAnalysisDataPicker))})
  for(i in 0:17) {
    output[[paste0("oneCoverBoxs",i)]] <- renderUI({h4(tags$b("Analysis Dataset: "), makeNice(input$zeroAnalysisDataPicker))})
    # print(c(ymd(dat2$maxtime)-years(1)),ymd(dat2$maxtime)))
    # print(paste0(ymd(date(dat2$maxtime)) - years(1)," - ",date(dat2$maxtime)))
    # TODO change the times to be data-derived
    # TODO make setField work
    # browser()
    # dat2$maxtime
    # dat2$mintime
    ...
    # }
    # paste0(dat2$mintime, " - ", date(dat2$maxtime))
  }
  ...
  # misc = dat$attributes %>% select(starts_with("clinic.misc")) %>% colnames()
  # updateSelectInput(session, "oneClinicMiscDropdown", choices = misc, selected = misc[1])
  # output$oneIndividualConditionTree <- renderTree({dat2$tree$clinColsL})
  ...
  updateSelectizeInput(session, "oneClinMulti", choices = dat2$ltcCols, selected = dat2$ltcCols[1:2],
                       options = list(
  ...
  output$oneActGraph2 <- renderPlotly({dat2$actGraph2 %>% arrange(cost_cumsum_prop, pop_prop) %>% 
  ...
  if(toLoad$widerDet) {
    ...
    updateSelectInput(session, "oneWidDetX", choices = detXvalues) # dat2$widerCols
  } else {
    output$uipyramidDeterminants <- renderUI({
