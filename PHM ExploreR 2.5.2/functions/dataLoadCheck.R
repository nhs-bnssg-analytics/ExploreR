...
landingUIPart1 <- function(dat, dat2) {
  ...
  if(class(dat2) == "try-error") {
    ...
    column(width = 12,
           ...
           p(paste0("There are ", format(nrow(dat$attributes), big.mark = ","), " individuals in Population Dataset. There are ", format(sum(dat$attributes$total_act), big.mark = ","), " points of activity across all PODs over the last 12 months.")),
  ...
  output$zeroAnalysisDataUI <- renderUI({
    lapply(1:length(dat2$AnalysisDataset$rules), function(x) {
      ...
      fluidRow(
        ...
        column(width = 2,
               actionButton(paste0("zeroAnalysisDataset", "Rule",x,"Delete"), "Delete Rule")
        )
      )
    })
  })
  ...
  output[[paste0("zeroAnalysisDatasetUI",x)]] <- renderUI({
    if(operator == "=") {
      ...
      column(width = 12,
             ...
             p(paste0("<= ",makeNice(input[[paste0("zeroAnalysisDatasetRule",x,"Field")]]), " <=")),
