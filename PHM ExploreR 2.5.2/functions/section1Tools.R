...
riskStratGroupSummaryBoxPlots <- function(data, input) {
  data$predicted_act_group <- factor(paste0("Group ",factor(data$predicted_act_group)), levels = c(paste0("Group ", 1:10)))
  ggplotly(
    data%>%ggplot(aes(x = predicted_act_group, y = predicted_act, group = predicted_act_group)) +
      geom_boxplot() +
      coord_flip() +
      ylab(makeNice(isolate(input$twoRisk1Var2))) +
      xlab("Risk Group")+
      ggtitle("Boxplots of Predicted Values")
  )
}
...
util_risk_strat <- function(dat, data) {
  ...
  d2 <- merge(dat$attributes%>% select_at("id"), data%>%select_at(c("id", "predicted_act_group")), all.x = T, sort = F)
