      ...
      if(isolate(input$twoClusterVar1) != isolate(input$twoClusterVar2)) {
        if(length(unique(dat$attributes[[input$twoClusterVar1]])) < 20 && length(unique(dat$attributes[[input$twoClusterVar2]])) < 20 ) {
          output$twoClusterTestPlot <- renderPlotly({
            ...
            # segment_proportion_bars_general(
            #   dat2 = isolate(
            #   dat$attributes %>%
            ...
            #     ungroup() #%>% mutate_at() ## TODO capitalise all segments (and y axis?)
            # ),
            # stacked = input$twoClustersegment_layout,
            # percent_or_freq = input$twoClustersegment_percent)
          })
  ...
  segment_proportion_bars_g <- function(dat2, stacked, percent_or_freq, fill_colours = "default", number_of_columns=2, focusName){
    ...
    if(stacked == "separate"){
      plot <- plot +
        ...
        # facet_wrap(~!!sym(isolate(input$twoClusterVar2)),
        #            ncol = number_of_columns
        # ) +
      facet_wrap(as.formula(paste("~", isolate(input$twoClusterVar2))),
                 ncol = number_of_columns
      ) +
        theme(legend.position = "none",
              panel.spacing.y=unit(-0.5,"lines")
...
# # will need to do something about input here - how to colour, etc.
    ...
    if(!is.null(input$twoRisk1Var1) & nrow(ntree[treeCols,]) > 0) {
      ...
      ## read in and mutate activity
        ...
        if(input$fiveRiskExcImpute == "Exclude") {
          ...
          if(class(act_data2) != "data.frame"  | class(act_data2) == "try-error" | class(act_data2) == "NULL") {
            NAcount <- lapply(act_data, function(x) {
              length(which(is.na(x)))/length(x) > 0.1
            })%>%unlist
            act_data<-act_data[,!NAcount]
        ...
        if(input$twoRisk1Var2 == "Likelihood of activity [0,1]") {
          ...
          # TODO review line below when I have a clear head
          ...
          dat2$segMem[[5]] <<- util_risk_strat(dat, act_data)
          # browser() # TODO add in defult value when excluding NAs
          showModal(
            modalDialog(
              h2("Graphing Results"),
              footer = list(),
 ...
 observeEvent(input$fiveRiskGGAdd, {
  ...
  # 
  #       p("A number of summary measures and graphs are produced as outputs (as well as the 'risk groups'):"),
  #       p("A summary of how many NAs were found, and subsequently excluded (if using exclusion)."),
  #       p("Summary statistics of the logistic (and if run) the linear regression performance.",
  ...
  #       p("Graphs for the logistic regression are an ", a(href = "https://acutecaretesting.org/en/articles/roc-curves-what-are-they-and-how-are-they-used", target = "_blank", "ROC curve"),
  #         ", and a confusion matrix summarising how many individuals had activity correctly predicted."
  #       ),
  #       p("If run, the linear regression provides a graph of predicted against test data. The closer this is to a straight line, the more accurate the model is."),
  #       p("Summaries of the stratified groups. Includes a table of a key measures, and a boxplot for easier comparison."),
  # 
  #       footer = tagList(
  #         modalButton("OK", icon = NULL)
  #       ),
