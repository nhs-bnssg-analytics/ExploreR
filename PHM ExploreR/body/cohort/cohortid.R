                              ...
                              box(width = 12, title = "Identification",
                                  ...
                                  fluidRow(
                                    ...
                                    column(width = 3, style = "margin-top: 25px;",
                                           actionButton("threeCohortNewUI","Add New Rule")
                                    ),
                                    column(width = 7, #style = "margin-top: 25px;",
                                           textInput("threeCohortDataFilterOrder", label = "Order of Operation(s):")
                                    ),
                                    column(width = 2, style = "margin-top: 25px;",
                                           p(" ", actionLink(inputId = "threeCHelp1",
                                                             icon = icon("info-circle"),
                                                             label = HTML(""))),
                                           bsTooltip("threeCHelp1", title = "Please write an expression from rules using rule numbers, brackets, ANDs and ORs. For example: 1 AND (2 OR 3)")
                                           
                                           # p(" ", actionLink(inputId = "threeCohortDataFilterHelp",
                                           #                   icon = icon("info-circle"),
                                           #                   label = HTML(""))),
                                           # bsTooltip("threeCohortDataFilterHelp", title = "Please write an expression from rules using ANDs and ORs. For example: '1 AND (2 OR 3)'"),
                                    ),
                                    br(),
                                    br(),
                                  ),
